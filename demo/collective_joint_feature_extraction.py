...
def parse_args():
    parser = argparse.ArgumentParser(description='HRNet inference on Collective Activity')
    # general
    ...
    parser.add_argument('--dataset_path', type=str, default='/home/honglu/Dataset/CAD/videos')
    parser.add_argument('--track_path', type=str, default='/home/honglu/Dataset/CAD/tracks_normalized.pkl')
    parser.add_argument('--save_path', type=str, default='/home/honglu/Dataset/CAD/joints')  
    ...
    for video_id in tqdm(videos):
        ...
        for clip_id in clips:
            ...
            for frame_id in sorted(tdata[(int(video_id), int(clip_id))]): 
                ...
                img_path = args.dataset_path + '/{}/{}/'.format(video_id, clip_id) + 'frame{}.jpg'.format(str(frame_id).zfill(4))
                ...
                for track_id, box in enumerate(tdata[(int(video_id), int(clip_id))][frame_id]):    
                    y1,x1,y2,x2 = box
                    box = [(int(x1*FRAMES_SIZE[int(video_id)][1]), int(y1*FRAMES_SIZE[int(video_id)][0])), 
                           (int(x2*FRAMES_SIZE[int(video_id)][1]), int(y2*FRAMES_SIZE[int(video_id)][0]))] 

                    pred_boxes.append(box)
                

                
                if not pred_boxes:  # Can not find people. 
                    print('video: {} clip: {} frame: {} pred_boxes is empty!'.format(video_id, clip_id, frame_id))
                    joint_output[frame_id] =  np.zeros((0, 17, 3))
                    ...
                    joint_output[frame_id] = np.array(this_frame_all_people)  # (N, 17, 3)
